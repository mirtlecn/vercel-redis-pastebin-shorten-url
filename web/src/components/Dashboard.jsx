...
export function Dashboard({ onLogout, token }) {
  ...
  async function loadItems() {
    try { setItems(sortItems(await apiRequest(token))); }
  ...
  async function remove(path) {
    try {
      ...
      setItems((v) => v.filter((item) => item.path !== path)); showToast('success', 'Deleted');
    } catch (error) { showToast('error', error.message); }
  }
  ...
  async function created(payload) {
    setResult(payload); setPage(1); await loadItems();
  }
