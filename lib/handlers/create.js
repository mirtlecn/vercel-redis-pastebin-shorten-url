...
async function handleTopicMutation(req, res, {
  ...
  if (!existingTopic && allowOverwrite) {
    await createTopic(redis, path, { title, titleProvided });
  } else if (existingTopic && allowOverwrite) {
    await refreshTopic(redis, path, { title, titleProvided });
  } else {
    await createTopic(redis, path, { title, titleProvided });
  }
...
async function handleJsonRequest(req, res, { allowOverwrite }) {
  ...
  try {
    if (inputType === 'md2html') {
      const topicBackLabel = resolvedTopicPath.isTopicItem
        ? await getTopicDisplayTitle(redis, resolvedTopicPath.topicName)
        : '';
